...
(function () {
  ...
  var blogPosts = document.querySelectorAll('[data-blog-categories]');
  var blogEmptyState = document.querySelector('.blog-filter-empty');

  if (blogFilters.length > 0 && blogPosts.length > 0) {
    blogFilters.forEach(function (filter) {
      filter.addEventListener('click', function () {
        var selected = filter.getAttribute('data-blog-filter');
        var visibleCount = 0;

        blogFilters.forEach(function (item) {
        ...
        blogPosts.forEach(function (post) {
          var categories = (post.getAttribute('data-blog-categories') || '').split(/\s+/);
          var visible = selected === 'all' || categories.indexOf(selected) !== -1;
          post.hidden = !visible;
          if (visible) visibleCount += 1;
        });

        if (blogEmptyState) blogEmptyState.hidden = visibleCount !== 0;
      });
    });
  }
